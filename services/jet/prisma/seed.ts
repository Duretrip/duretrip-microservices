...
async function main() {
  try {
    ...
    const range1 = await prisma.range.upsert({
      where: { id:1, name: 'Short Range (Up to 2000 Nautical Miles)' },
    ...
    const range2 = await prisma.range.upsert({
      where: { id:2, name: 'Medium Range (2000-5000 Nautical Miles)' },
    ...
    const range3 = await prisma.range.upsert({
      where: { id:3, name: 'Long Range (5000+ Nautical Miles)' },
    ...
    const capacity1 = await prisma.capacity.upsert({
      where: { id: 1, name: 'Small jet (Up to 6 passengers)' },
    ...
    const capacity2 = await prisma.capacity.upsert({
      where: { id: 2, name: 'Midsize jet (7-10 passengers)' },
    ...
    const capacity3 = await prisma.capacity.upsert({
      where: { id: 3, name: 'Large Jet (11-19 passengers)' },
